...
func (s *Server) handleLoginHomeowner(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		s.logLoginAttempt(r.Context(), body.Username, body.Password, RoleHomeowner, false)
	...
	if u.Role != RoleHomeowner || !u.PasswordHash.Valid || !auth.CheckPassword(u.PasswordHash.String, body.Password) {
		...
		s.logLoginAttempt(r.Context(), body.Username, body.Password, RoleHomeowner, false)
		s.writeError(w, 401, "invalid password or role")
		return
	}
	...
	s.logLoginAttempt(r.Context(), body.Username, body.Password, RoleHomeowner, true)
...
func (s *Server) handleLoginGuest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Username doesn't exist - log the failed attempt
		s.logLoginAttempt(r.Context(), body.Username, body.PIN, RoleGuest, false)
		s.writeError(w, 401, "invalid username")
		return
	}
	...
	// Log successful login
	s.logLoginAttempt(r.Context(), body.Username, body.PIN, RoleGuest, true)
	_ = json.NewEncoder(w).Encode(sess)
}
...
func (s *Server) handleLoginTechnician(w http.ResponseWriter, r *http.Request) {
	...
	// Log successful login
	s.logLoginAttempt(r.Context(), body.Username, body.Password, RoleTechnician, true)
	_ = json.NewEncoder(w).Encode(sess)
}
...
func (s *Server) logLoginAttempt(ctx context.Context, username, password, role string, success bool) {
